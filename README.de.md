...
- Datenbank (inkl. `-wal`/`-shm`), automatische Backups und die generierte Caddy-Konfiguration nur für den Eigentümer lesbar (`0600`, Backup-Verzeichnis `0700`) — bestehende Installationen werden beim nächsten Start angepasst
...
| `GC_RATE_LIMIT_GATEWAY` | `300` | Max. Gateway-API-Requests (Heartbeat, Config-Check, …) pro 15 Min, je Gateway |
...
| `GC_CADDY_STREAM_CLOSE_DELAY` | `1h` | Wie lange WebSockets (Web-Terminal, RDP) einen Caddy-Config-Reload überleben; `0` schließt sie sofort |
