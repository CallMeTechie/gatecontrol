...
function validateConfig() {
  ...
  if (!config.app.secret) {
    errors.push('GC_SECRET must be set (generated by entrypoint.sh and persisted to /data/.session_secret)');
  }
  if (!config.encryption.key) {
    errors.push('GC_ENCRYPTION_KEY must be set — required to encrypt peer private keys and TOTP secrets');
  }
  if (config.encryption.key && !/^[0-9a-fA-F]{64}$/.test(config.encryption.key)) {
    errors.push('GC_ENCRYPTION_KEY must be exactly 64 hex characters (32 bytes for AES-256)');
  }
