...
describe('portal_tls_home_internal: forceInternalDomains', () => {
  it('puts home.example.com under internal issuer when forced, and app.example.com under ACME', () => {
    ...
    // home.example.com must appear in an internal-CA policy
    const internalPolicy = policies.find(
      p =>
        Array.isArray(p.subjects) &&
        p.subjects.includes('home.example.com') &&
        Array.isArray(p.issuers) &&
        p.issuers.some(i => i.module === 'internal'),
    );
    assert.ok(
      internalPolicy,
      'home.example.com must be in an internal-issuer policy (forceInternalDomains override)',
    );
    ...
    const acmePolicy = policies.find(
      p =>
        ...
        p.subjects.includes('home.example.com') &&
        Array.isArray(p.issuers) &&
        p.issuers.some(i => i.module === 'acme'),
    );
    assert.equal(
      acmePolicy,
      undefined,
    ...
    const appAcme = policies.find(
      p =>
        Array.isArray(p.subjects) &&
        p.subjects.includes('app.example.com') &&
        Array.isArray(p.issuers) &&
        p.issuers.some(i => i.module === 'acme'),
    );
    assert.ok(appAcme, 'app.example.com must be in the ACME policy');
  });
