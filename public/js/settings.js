...
(function () {
  ...
  document.getElementById('btn-clear-logs').addEventListener('click', async function() {
    ...
    btnLoading(btn);
    try {
      var data = await api.post('/api/settings/clear-logs');
      if (data.ok) {
  ...
  async function loadWebhooks() {
    try {
      var data = await api.get('/api/webhooks');
  ...
  if (webhooksList) {
    webhooksList.addEventListener('click', async function(e) {
      ...
      if (!btn) return;
      var action = btn.dataset.whAction;
      var id = btn.dataset.whId;

      if (action === 'test') {
        try {
          var data = await api.post('/api/webhooks/' + id + '/test');
  ...
  var btnAddWebhook = document.getElementById('btn-add-webhook');
  if (btnAddWebhook) {
    btnAddWebhook.addEventListener('click', async function() {
      var url = document.getElementById('webhook-url').value.trim();
      var description = document.getElementById('webhook-desc').value.trim();
      if (!url) return alert('Webhook URL is required');
      try {
        var data = await api.post('/api/webhooks', { url: url, description: description, events: '*' });
        if (data.ok) {
  ...
  document.getElementById('btn-backup-download').addEventListener('click', async function() {
    try {
      var resp = await fetch('/api/settings/backup', { credentials: 'same-origin' });
      if (!resp.ok) throw new Error('Download failed');
      var blob = await resp.blob();
      var url = URL.createObjectURL(blob);
      var a = document.createElement('a');
      a.href = url;
      var match = (resp.headers.get('Content-Disposition') || '').match(/filename="(.+)"/);
  ...
  document.getElementById('backup-file-input').addEventListener('change', async function(e) {
    ...
    var preview = document.getElementById('backup-preview');
    var restoreBtn = document.getElementById('btn-backup-restore');
    ...
    try {
      var resp = await fetch('/api/settings/restore/preview', {
        method: 'POST',
  ...
  document.getElementById('btn-backup-restore').addEventListener('click', async function() {
    ...
    try {
      var resp = await fetch('/api/settings/restore', {
        method: 'POST',
        credentials: 'same-origin',
        headers: { 'X-CSRF-Token': window.GC.csrfToken },
        body: formData,
